...
function displayWeather() {
  ...
  Lon.innerHTML = `<p> Долгота: ${weather.lon}</p>`;
  Lat.innerHTML = `<p> Ширина: ${weather.lat}</p>`;
  Rise.innerHTML = `<p> Восход: ${RiseTime} AM</p>`;
  Seet.innerHTML = `<p> Закат: ${SetTime} PM</p>`;
  MinT.innerHTML = `<p> МинТемпература: ${weather.minTemp.value} &deg;C</p>`;
  MaxT.innerHTML = `<p> МаксТемпература: ${weather.maxTemp.value} &deg;C</p>`;
  Press.innerHTML = `<p> Давление: ${weather.press} hPa`;
  Humid.innerHTML = `<p> Влажность: ${weather.humid}%</p>`;
  WindSpd.innerHTML = `<p> СкоростьВетра: ${weather.speedW} m/s</p>`;
  WindDir.innerHTML = `<p> Направление: ${weather.dirW}&deg;</p>`;
}
